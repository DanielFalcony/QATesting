...
class TestPositive:
    ...
    def test_step3(self, clear_folders, make_files, stat_log):
        ...
        """
        res = []
        res.append(checkout(f"cd {data['folder_in']}; 7z a -t{data['arx_type']} {data['folder_out']}/arx2", "Everything is Ok"))
        for item in make_files:
    ...
    def test_step4(self, clear_folders, make_files, stat_log):
        ...
        res = []
        res.append(checkout(f"cd {data['folder_in']}; 7z a -t{data['arx_type']} {data['folder_out']}/arx2", "Everything is Ok"))
        res.append(checkout(f"cd {data['folder_out']}; 7z e arx2.{data['arx_type']} -o{data['folder_ext']} -y", "Everything is Ok"))
        for item in make_files:
    ...
    def test_step5(self, clear_folders, make_files, make_subfolder, stat_log):
        ...
        """
        res = []
        res.append(checkout(f"cd {data['folder_in']}; 7z a -t{data['arx_type']} {data['folder_out']}/arx", "Everything is Ok"))
        res.append(checkout(f"cd {data['folder_out']}; 7z x arx.{data['arx_type']} -o{data['folder_ext2']} -y", "Everything is Ok"))
        for item in make_files:
